...
\[ r \equiv_n a \]
Portanto, o resíduo é \uline{único} e \uline{mínimo}. \\[10pt]
...
\]
Sejam $m, n \in \mathbb{Z}$ co-primos. A congruência $x \equiv_{(m \cdot n)} a$ é equivalente ao sistema chinês
\[
  \begin{cases}
    x \equiv_m a \\
    x \equiv_n a
...
\begin{itemize}
  \item associativa: $\forall\: a,b,c \in A: a * (b * c) = (a * b) * c$
  \item elemento neutro: $\exists\: \lambda \in A, \forall\: a \in A: a * \lambda = \lambda * a = a$
  \item inverso: $\forall\: a \in A, \exists\: a' \in A: a * a' = a' * a = \lambda$
  \item comutatividade: $\forall\: a,b \in A: a * b = b * a$
\end{itemize}
\vspace{10pt}
...
\begin{itemize}
  ...
  \item $+$ possui inverso.
\end{itemize}
Se, além disso, $(A, +, \cdot)$ é tal que $(A \setminus \{0\}, \cdot)$ possui inverso, dizemos que $A$ é um corpo.
...
\begin{align*}
  & \forall\: {[a]}_m, {[b]}_m \in \mathbb{Z}_{/m}: {[a]}_m + {[b]}_m := {[a + b]}_m \\
  & \forall\: {[a]}_m, {[b]}_m \in \mathbb{Z}_{/m}: {[a]}_m \cdot {[b]}_m := {[a \cdot b]}_m
\end{align*}
\uline{Teorema}: $(\mathbb{Z}_{/m}, +, \cdot)$ é um anel comutativo unitário $\forall\: m \in \mathbb{Z}$. \\[5pt]
\uline{Teorema}: Seja $\bar{a} \cdot \bar{c} = \bar{b} \cdot \bar{c}$ em $Z_{/m}$. Se $c$ e $m$ são co-primos, então $\bar{a} = \bar{b}$
